...
def gen_position_info_legacy(report:Report)-> pd.DataFrame:
    from .backtest import get_rebalance_date
    from .data import DatasetsHandler

    last_buy_date = report.buy_list.index[-2]
    this_buy_date = report.buy_list.index[-1]
    next_buy_date = pd.Series(get_rebalance_date(report.rebalance, start=this_buy_date, end=(dt.datetime.today()+pd.DateOffset(years=1)))).loc[1]

    last_buy_idx = report.portfolio_df.index.get_loc(report.buy_list.index[-1])
    to_buy = report.portfolio_df\
        .iloc[last_buy_idx, :]\
        .replace({0:np.nan})\
        .dropna()\
        .to_frame('weight')\
        ...
        .set_index(['action', 'stock_id'])\
        .assign(buy_date=last_buy_date)\
        .assign(sell_date=this_buy_date)
    to_sell = report.portfolio_df\
        .iloc[last_buy_idx-1, :]\
        .replace({0:np.nan})\
        .dropna()\
        .to_frame('weight')\
        ...
        .reset_index(drop=False)\
        .set_index(['action', 'stock_id'])\
        .assign(buy_date=this_buy_date)\
        .assign(sell_date=next_buy_date)
    to_trade = pd.concat([to_buy, to_sell])

    if to_trade.empty:
        return pd.DataFrame()

    filters=[('stock_id', 'in', to_trade.index.droplevel(0).to_list())]
    ...
    return pd.concat([
        to_trade.reset_index(drop=False).set_index(['stock_id']),
        ch_name,
        q_report_release_date,
        m_rev_release_date,
        trading_notes,
    ...
    .set_index(['action', 'stock_id'])\
    [[
        'name', 'weight', 'buy_date', 'sell_date', 
        '警示狀態', '漲跌停',
        '前次成交量_K', '前次成交額_M', 
        '季報公佈日期', '月營收公佈日期', 
        '板塊別', '主產業別',
