...
A quantitative research and trading python package in Taiwan stock market.
...
## backtesting
The `backtesting()` function under the `scytheq.backtest` module tests trading strategies and returns a `Strategy` instance with analysis tools including:
...
The `multi_backtesting()` function under the `scytheq.backtest` module allows you to combine multiple strategies into a portfolio strategy. It returns a `MultiStrategy` instance with analysis tools including:
...
```python
from scytheq.trade import Position
...
```python
from scytheq.trade import Portfolio
...
```python
from scytheq.trade import SinoPacAccount, PortfolioManager
...
```bash
pip install git+https://github.com/yourusername/scytheq.git
```
...
```bash
git clone https://github.com/yourusername/scytheq.git
cd scytheq
pip install -e .
```
...
Before using ScytheQ, ensure you have:
...
```python
import scytheq
scytheq.set_config_dir('/path/to/config') # if not set, the default is ./config
```
